...
public:
	...
	virtual std::string getClassName() const { return "CastExpr"; }
	...
	typedef boost::shared_ptr<CastExpr> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<CastExpr>(n); }
	template<typename T> static Ptr needFrom(const T& n) {
		Ptr r = boost::dynamic_pointer_cast<CastExpr>(n);
		if (!r)
			throw std::runtime_error("Node " + n->getId().str() + " (a " + n->getClassName() + ") cannot be dynamically cast to CastExpr.");
		return r;
	}
...
protected:
	...
	// interfaces
	GraphInterfaceImpl<CastExpr> interfaceGraph;
	TypeInterfaceImpl<CastExpr> interfaceType;
};
