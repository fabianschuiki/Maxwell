...
class MutableTypeExpr : public Node
{
public:
	// constructor
	MutableTypeExpr();
	...
	virtual std::string getClassName() const { return "MutableTypeExpr"; }
	...
	typedef boost::shared_ptr<MutableTypeExpr> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<MutableTypeExpr>(n); }
	template<typename T> static Ptr needFrom(const T& n) {
		Ptr r = boost::dynamic_pointer_cast<MutableTypeExpr>(n);
		if (!r)
			throw std::runtime_error("Node " + n->getId().str() + " (a " + n->getClassName() + ") cannot be dynamically cast to MutableTypeExpr.");
		return r;
	}
...
protected:
	...
	// interfaces
	GraphInterfaceImpl<MutableTypeExpr> interfaceGraph;
	TypeExprInterfaceImpl<MutableTypeExpr> interfaceTypeExpr;
};
