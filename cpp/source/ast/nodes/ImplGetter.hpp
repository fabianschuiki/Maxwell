...
public:
	...
	virtual string getClassName() const { return "ImplGetter"; }
	...
	{
		const shared_ptr<ImplGetter>& other = boost::dynamic_pointer_cast<ImplGetter>(o);
	...
	typedef boost::shared_ptr<ImplGetter> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<ImplGetter>(n); }
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<ImplGetter>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to ImplGetter."); return r; }
protected:
	NodeRef graphPrev;
	string name;
	NodeVector in;
	...
	// Interfaces
	GraphInterfaceImpl<ImplGetter> interfaceGraph;
	CallableInterfaceImpl<ImplGetter> interfaceCallable;
	NamedInterfaceImpl<ImplGetter> interfaceNamed;
};
