...
public:
	...
	virtual string getClassName() const { return "IndexOpExpr"; }
	...
	void setIndexee(const NodePtr& v)
	{
		if (v && !v->implements(kTypeInterface)) {
			throw runtime_error("'indexee' of " + id.str() + " needs to be of kind {} or implement interface {Type}, got " + v->getClassName() + " (" + v->getId().str() + ") instead.");
		}
		if (!equal(v, indexee)) {
			modify("indexee");
			indexee = v;
	...
	const NodePtr& getIndexee(bool required = true)
	{
		const NodePtr& v = indexee;
		if (required && !v) {
			throw runtime_error("Node " + getId().str() + " is required to have indexee set to a non-null value.");
		}
		return v;
	}

	void setIndex(const NodePtr& v)
	{
		if (v && !v->implements(kTypeInterface)) {
			throw runtime_error("'index' of " + id.str() + " needs to be of kind {} or implement interface {Type}, got " + v->getClassName() + " (" + v->getId().str() + ") instead.");
		}
		if (!equal(v, index)) {
			modify("index");
			index = v;
	...
	const NodePtr& getIndex(bool required = true)
	{
		const NodePtr& v = index;
		if (required && !v) {
			throw runtime_error("Node " + getId().str() + " is required to have index set to a non-null value.");
		}
		return v;
	}
	...
	virtual string describe(int depth = -1)
	{
		...
		if (this->indexee) b << endl << "  \033[1mindexee\033[0m = " << indent(this->indexee->describe(depth-1));
		if (this->index) b << endl << "  \033[1mindex\033[0m = " << indent(this->index->describe(depth-1));
	...
	typedef boost::shared_ptr<IndexOpExpr> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<IndexOpExpr>(n); }
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<IndexOpExpr>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to IndexOpExpr."); return r; }
protected:
	NodeRef graphPrev;
	NodePtr indexee;
	NodePtr index;
	...
	// Interfaces
	GraphInterfaceImpl<IndexOpExpr> interfaceGraph;
	TypeInterfaceImpl<IndexOpExpr> interfaceType;
};
