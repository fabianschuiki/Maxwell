...
class InterfaceQualifier : public Node
{
public:
	InterfaceQualifier() : Node(),
		interfaceGraph(this) {}
	...
	typedef boost::shared_ptr<InterfaceQualifier> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<InterfaceQualifier>(n); }
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<InterfaceQualifier>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to InterfaceQualifier."); return r; }
protected:
	NodeRef graphPrev;
	NodeVector stmts;

	// Interfaces
	GraphInterfaceImpl<InterfaceQualifier> interfaceGraph;
};
