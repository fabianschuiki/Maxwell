...
class StructureInheritance extends \RepositoryNodeObject implements \EqualInterface, GraphInterface
{
	/* PROPERTIES */
	...
	public function getFragment($name)
	{
		switch ($name) {
			case "tree": return array(
				array("name" => "type", "type" => "NamedType"));
	...
	{
		return "StructureInheritance";
	}
	
	public function isEqualTo($x)
	{
		if (!$x instanceof self) {
			throw new \InvalidArgumentException('x needs to be an instance of StructureInheritance as well.');
		}
		// tree fragment
		if (!$this->tree_loaded) {
	...
	{
		if (!$type instanceof NamedType && !$type instanceof \RepositoryObjectReference) {
			throw new \InvalidArgumentException('Object '.$this->getId().' needs type to be an instance of NamedType or \RepositoryObjectReference');
		}
