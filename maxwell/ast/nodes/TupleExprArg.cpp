...
#include <sstream>
#include <stdexcept>
using ast::TupleExprArg;
using ast::NodePtr;
using ast::NodeVector;

TupleExprArg::TupleExprArg() : Node(),
	interfaceCallArg(this),
	interfaceGraph(this),
	interfaceNamed(this) {}

bool TupleExprArg::isKindOf(Kind k) {
	if (Node::isKindOf(k)) return true;
	return k == kTupleExprArg;
}

bool TupleExprArg::implements(Interface i) {
...
NodePtr TupleExprArg::copy() {
	Ptr c (new TupleExprArg);
...
bool TupleExprArg::equalTo(const NodePtr& o) {
	const shared_ptr<TupleExprArg>& other = boost::dynamic_pointer_cast<TupleExprArg>(o);
...
std::string TupleExprArg::describe(int depth) {
	std::stringstream str, b;
	if (depth == 0) return "TupleExprArg{…}";
	str << "TupleExprArg{";
...
void TupleExprArg::setGraphPrev(const NodePtr& v) {
	if (!v && graphPrev) {
		modify("graphPrev");
		graphPrev.reset();
	}
...
void TupleExprArg::setGraphPrev(const NodeId& v) {
	if (v != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
...
const NodePtr& TupleExprArg::getGraphPrev(bool required) {
...
void TupleExprArg::setName(const std::string& v) {
	if (!equal(v, name)) {
		modify("name");
		name = v;
	}
...
const std::string& TupleExprArg::getName(bool required) {
...
void TupleExprArg::setExpr(const NodePtr& v) {
...
const NodePtr& TupleExprArg::getExpr(bool required) {
...
void TupleExprArg::encode(Encoder& e) {
	e.encode(this->graphPrev);
	e.encode(this->name);
	e.encode(this->expr);
}

void TupleExprArg::decode(Decoder& d) {
...
void TupleExprArg::updateHierarchyOfChildren() {
	if (this->expr) this->expr->updateHierarchy(id + "expr", repository, this);
}

const NodePtr& TupleExprArg::resolvePath(const std::string& path) {
...
NodeVector TupleExprArg::getChildren() {
	NodeVector v;
