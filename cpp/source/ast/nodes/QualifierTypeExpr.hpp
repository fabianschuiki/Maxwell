...
class QualifierTypeExpr : public Node
{
public:
	QualifierTypeExpr() : Node(),
		interfaceGraph(this),
	...
	virtual string getClassName() const { return "QualifierTypeExpr"; }
	...
	{
		if (depth == 0) return "QualifierTypeExpr{…}";
		stringstream str, b;
		str << "QualifierTypeExpr{";
	...
	{
		const shared_ptr<QualifierTypeExpr>& other = boost::dynamic_pointer_cast<QualifierTypeExpr>(o);
	...
	typedef boost::shared_ptr<QualifierTypeExpr> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<QualifierTypeExpr>(n); }
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<QualifierTypeExpr>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to QualifierTypeExpr."); return r; }
protected:
	NodeRef graphPrev;
	...
	// Interfaces
	GraphInterfaceImpl<QualifierTypeExpr> interfaceGraph;
	TypeExprInterfaceImpl<QualifierTypeExpr> interfaceTypeExpr;
};
