...
bool CallExpr::isKindOf(Kind k) {
	if (Node::isKindOf(k)) return true;
	return k == kCallExpr;
}

bool CallExpr::implements(Interface i) {
...
bool CallExpr::equalTo(const NodePtr& o) {
...
std::string CallExpr::describe(int depth) {
	...
	if (this->context) b << "\n  \033[1mcontext\033[0m = " << indent(this->context->describe(depth-1));
...
void CallExpr::setGraphPrev(const NodePtr& v) {
	if (!v && graphPrev) {
		modify("graphPrev");
		graphPrev.reset();
	}
...
void CallExpr::setGraphPrev(const NodeId& v) {
	if (v != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
...
const NodePtr& CallExpr::getGraphPrev(bool required) {
...
void CallExpr::setName(const std::string& v) {
	if (!equal(v, name)) {
		modify("name");
		name = v;
	}
...
const std::string& CallExpr::getName(bool required) {
...
void CallExpr::setContext(const NodePtr& v) {
	if (!equal(v, context)) {
		modify("context");
		context = v;
	}
...
const NodePtr& CallExpr::getContext(bool required) {
	const NodePtr& v = context;
	if (required && !v) {
		throw std::runtime_error("Node " + getId().str() + " is required to have context set to a non-null value.");
	}
	return v;
}


void CallExpr::setArgs(const NodeVector& v) {
	if (!equal(v, args)) {
		modify("args");
		args = v;
	}
...
const NodeVector& CallExpr::getArgs(bool required) {
	const NodeVector& v = args;
	return v;
}


void CallExpr::setCallName(const std::string& v) {
	if (!equal(v, callName)) {
		modify("callName");
		callName = v;
	}
...
const std::string& CallExpr::getCallName(bool required) {
...
void CallExpr::setCallArgs(const NodeVector& v) {
	if (!equal(v, callArgs)) {
		modify("callArgs");
		callArgs = v;
	}
...
const NodeVector& CallExpr::getCallArgs(bool required) {
	const NodeVector& v = callArgs;
	return v;
}


void CallExpr::setCallCandidates(const NodeVector& v) {
...
const NodeVector& CallExpr::getCallCandidates(bool required) {
	const NodeVector& v = callCandidates;
	return v;
}


void CallExpr::setSelectedCallCandidate(const NodePtr& v) {
...
void CallExpr::setSelectedCallCandidate(const NodeId& v) {
...
const NodePtr& CallExpr::getSelectedCallCandidate(bool required) {
...
void CallExpr::setPossibleType(const NodePtr& v) {
...
const NodePtr& CallExpr::getPossibleType(bool required) {
...
void CallExpr::setRequiredType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType)) {
...
const NodePtr& CallExpr::getRequiredType(bool required) {
...
void CallExpr::setActualType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType)) {
...
const NodePtr& CallExpr::getActualType(bool required) {
...
void CallExpr::encode(Encoder& e) {
...
void CallExpr::decode(Decoder& d) {
...
const NodePtr& CallExpr::resolvePath(const std::string& path) {
...
NodeVector CallExpr::getChildren() {
	NodeVector v;
