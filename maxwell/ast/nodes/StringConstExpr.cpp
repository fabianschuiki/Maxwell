...
#include <sstream>
#include <stdexcept>
using ast::StringConstExpr;
using ast::NodePtr;
using ast::NodeVector;

StringConstExpr::StringConstExpr() : Node(),
	interfaceGraph(this),
	interfaceType(this) {}

bool StringConstExpr::isKindOf(Kind k) {
	if (Node::isKindOf(k)) return true;
	return k == kStringConstExpr;
}

bool StringConstExpr::implements(Interface i) {
...
NodePtr StringConstExpr::copy() {
	Ptr c (new StringConstExpr);
...
bool StringConstExpr::equalTo(const NodePtr& o) {
...
std::string StringConstExpr::describe(int depth) {
...
void StringConstExpr::setGraphPrev(const NodePtr& v) {
	if (!v && graphPrev) {
		modify("graphPrev");
		graphPrev.reset();
	}
...
void StringConstExpr::setGraphPrev(const NodeId& v) {
	if (v != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
...
const NodePtr& StringConstExpr::getGraphPrev(bool required) {
...
void StringConstExpr::setPossibleType(const NodePtr& v) {
...
const NodePtr& StringConstExpr::getPossibleType(bool required) {
...
void StringConstExpr::setRequiredType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType) && !v->isKindOf(kNativeType) && !v->isKindOf(kMutableType) && !v->isKindOf(kMutableCastType) && !v->isKindOf(kImmutableCastType)) {
...
const NodePtr& StringConstExpr::getRequiredType(bool required) {
...
void StringConstExpr::setActualType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType) && !v->isKindOf(kNativeType) && !v->isKindOf(kMutableType) && !v->isKindOf(kMutableCastType) && !v->isKindOf(kImmutableCastType)) {
...
const NodePtr& StringConstExpr::getActualType(bool required) {
...
void StringConstExpr::setValue(const std::string& v) {
	if (!equal(v, value)) {
		modify("value");
		value = v;
	}
...
const std::string& StringConstExpr::getValue(bool required) {
...
void StringConstExpr::encode(Encoder& e) {
...
void StringConstExpr::decode(Decoder& d) {
...
void StringConstExpr::updateHierarchyOfChildren() {
...
const NodePtr& StringConstExpr::resolvePath(const std::string& path) {
