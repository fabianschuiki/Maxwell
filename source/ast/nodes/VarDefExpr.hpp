...
public:
	...
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<VarDefExpr>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to VarDefExpr."); return r; }
protected:
	NodeRef graphPrev;
