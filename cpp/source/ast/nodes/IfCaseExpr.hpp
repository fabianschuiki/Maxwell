...
public:
	...
	void setOtherwise(const NodePtr& v)
	{
		if (v && !v->implements(kTypeInterface)) {
			throw runtime_error("'otherwise' of " + id.str() + " needs to be of kind {} or implement interface {Type}, got " + v->getClassName() + " (" + v->getId().str() + ") instead.");
		}
	...
	const NodePtr& getOtherwise(bool required = true)
	{
		...
		if (required && !v) {
			throw runtime_error("Node " + getId().str() + " is required to have otherwise set to a non-null value.");
		}
		return v;
	}
	...
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<IfCaseExpr>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to IfCaseExpr."); return r; }
protected:
	NodeVector conds;
	NodePtr otherwise;
	NodeRef graphPrev;
