/* Automatically generated by ast-gen. DO NOT MODIFY. */
#include "IfCaseExprCond.hpp"
#include "ast/Coder.hpp"
#include <cstdio>
#include <sstream>
#include <stdexcept>
using ast::IfCaseExprCond;
using ast::NodePtr;
using ast::NodeVector;

IfCaseExprCond::IfCaseExprCond() : Node(),
	interfaceGraph(this) {}

bool IfCaseExprCond::isKindOf(Kind k) {
	if (Node::isKindOf(k)) return true;
	return k == kIfCaseExprCond;
}

bool IfCaseExprCond::implements(Interface i) {
	if (Node::implements(i)) return true;
	if (i == kGraphInterface) return true;
	return false;
}

NodePtr IfCaseExprCond::copy() {
	Ptr c (new IfCaseExprCond);
	Node::copy(this->expr, c->expr);
	Node::copy(this->cond, c->cond);
	Node::copy(this->graphPrev, c->graphPrev);
	return c;
}

bool IfCaseExprCond::equalTo(const NodePtr& o) {
	const shared_ptr<IfCaseExprCond>& other = boost::dynamic_pointer_cast<IfCaseExprCond>(o);
	if (!other) return false;
	if (!equal(this->expr, other->expr)) return false;
	if (!equal(this->cond, other->cond)) return false;
	if (!equal(this->graphPrev, other->graphPrev)) return false;
	return true;
}

std::string IfCaseExprCond::describe(int depth) {
	std::stringstream str, b;
	if (depth == 0) return "IfCaseExprCond{…}";
	str << "IfCaseExprCond{";
	if (this->expr) b << "\n  \033[1mexpr\033[0m = " << indent(this->expr->describe(depth-1));
	if (this->cond) b << "\n  \033[1mcond\033[0m = " << indent(this->cond->describe(depth-1));
	if (this->graphPrev) b << "\n  \033[1mgraphPrev\033[0m = \033[36m" << this->graphPrev.id << "\033[0m";
	string bs = b.str();
	if (!bs.empty()) str << bs << '\n';
	str << "}";
	return str.str();
}


void IfCaseExprCond::setExpr(const NodePtr& v) {
	if (v && !v->implements(kTypeInterface)) {
		throw std::runtime_error("'expr' of " + id.str() + " needs to be of kind {} or implement interface {Type}, got " + v->getClassName() + " (" + v->getId().str() + ") instead.");
	}
	if (!equal(v, expr)) {
		modify("expr");
		expr = v;
	}
}

const NodePtr& IfCaseExprCond::getExpr(bool required) {
	const NodePtr& v = expr;
	if (required && !v) {
		throw std::runtime_error("Node " + getId().str() + " is required to have expr set to a non-null value.");
	}
	return v;
}


void IfCaseExprCond::setCond(const NodePtr& v) {
	if (v && !v->implements(kTypeInterface)) {
		throw std::runtime_error("'cond' of " + id.str() + " needs to be of kind {} or implement interface {Type}, got " + v->getClassName() + " (" + v->getId().str() + ") instead.");
	}
	if (!equal(v, cond)) {
		modify("cond");
		cond = v;
	}
}

const NodePtr& IfCaseExprCond::getCond(bool required) {
	const NodePtr& v = cond;
	if (required && !v) {
		throw std::runtime_error("Node " + getId().str() + " is required to have cond set to a non-null value.");
	}
	return v;
}


void IfCaseExprCond::setGraphPrev(const NodePtr& v) {
	if (!v && graphPrev) {
		modify("graphPrev");
		graphPrev.reset();
	}
	if (!graphPrev || v->getId() != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
}

void IfCaseExprCond::setGraphPrev(const NodeId& v) {
	if (v != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
}

const NodePtr& IfCaseExprCond::getGraphPrev(bool required) {
	const NodePtr& v = graphPrev.get(repository);
	if (required && !v) {
		throw std::runtime_error("Node " + getId().str() + " is required to have graphPrev set to a non-null value.");
	}
	return v;
}


void IfCaseExprCond::encode(Encoder& e) {
	e.encode(this->expr);
	e.encode(this->cond);
	e.encode(this->graphPrev);
}

void IfCaseExprCond::decode(Decoder& d) {
	d.decode(this->expr);
	d.decode(this->cond);
	d.decode(this->graphPrev);
}


void IfCaseExprCond::updateHierarchyOfChildren() {
	if (this->expr) this->expr->updateHierarchy(id + "expr", repository, this);
	if (this->cond) this->cond->updateHierarchy(id + "cond", repository, this);
}

const NodePtr& IfCaseExprCond::resolvePath(const std::string& path) {
	size_t size = path.size();
	// .*
	if (true) {
		// cond.*
		if (size >= 4 && path[0] == 'c' && path[1] == 'o' && path[2] == 'n' && path[3] == 'd') {
			// cond
			if (size == 4) {
				return getCond();
			} else if (path[4] == '.') {
				return getCond()->resolvePath(path.substr(5));
			}
		}
		// expr.*
		if (size >= 4 && path[0] == 'e' && path[1] == 'x' && path[2] == 'p' && path[3] == 'r') {
			// expr
			if (size == 4) {
				return getExpr();
			} else if (path[4] == '.') {
				return getExpr()->resolvePath(path.substr(5));
			}
		}
		// graphPrev.*
		if (size >= 9 && path[0] == 'g' && path[1] == 'r' && path[2] == 'a' && path[3] == 'p' && path[4] == 'h' && path[5] == 'P' && path[6] == 'r' && path[7] == 'e' && path[8] == 'v') {
			// graphPrev
			if (size == 9) {
				return getGraphPrev();
			} else if (path[9] == '.') {
				return getGraphPrev()->resolvePath(path.substr(10));
			}
		}
	}
	throw std::runtime_error("Node path '" + path + "' does not point to a node or array of nodes.");
}

NodeVector IfCaseExprCond::getChildren() {
	NodeVector v;
	if (const NodePtr& n = this->getExpr(false)) v.push_back(n);
	if (const NodePtr& n = this->getCond(false)) v.push_back(n);
	return v;
}

