...
#include <sstream>
#include <stdexcept>
using ast::TypelessVarDefExpr;
using ast::NodePtr;
using ast::NodeVector;

TypelessVarDefExpr::TypelessVarDefExpr() : Node(),
	interfaceGraph(this),
	interfaceNamed(this),
	interfaceType(this),
	interfaceVariable(this) {}

bool TypelessVarDefExpr::isKindOf(Kind k) {
	if (Node::isKindOf(k)) return true;
	return k == kTypelessVarDefExpr;
}

bool TypelessVarDefExpr::implements(Interface i) {
...
NodePtr TypelessVarDefExpr::copy() {
	Ptr c (new TypelessVarDefExpr);
...
bool TypelessVarDefExpr::equalTo(const NodePtr& o) {
	const shared_ptr<TypelessVarDefExpr>& other = boost::dynamic_pointer_cast<TypelessVarDefExpr>(o);
...
std::string TypelessVarDefExpr::describe(int depth) {
	std::stringstream str, b;
	if (depth == 0) return "TypelessVarDefExpr{…}";
	str << "TypelessVarDefExpr{";
...
void TypelessVarDefExpr::setGraphPrev(const NodePtr& v) {
	if (!v && graphPrev) {
		modify("graphPrev");
		graphPrev.reset();
	}
...
void TypelessVarDefExpr::setGraphPrev(const NodeId& v) {
	if (v != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
...
const NodePtr& TypelessVarDefExpr::getGraphPrev(bool required) {
...
void TypelessVarDefExpr::setPossibleType(const NodePtr& v) {
...
const NodePtr& TypelessVarDefExpr::getPossibleType(bool required) {
...
void TypelessVarDefExpr::setRequiredType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType)) {
...
const NodePtr& TypelessVarDefExpr::getRequiredType(bool required) {
...
void TypelessVarDefExpr::setActualType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType)) {
...
const NodePtr& TypelessVarDefExpr::getActualType(bool required) {
...
void TypelessVarDefExpr::setName(const std::string& v) {
	if (!equal(v, name)) {
		modify("name");
		name = v;
	}
...
const std::string& TypelessVarDefExpr::getName(bool required) {
...
void TypelessVarDefExpr::setTypeExpr(const NodePtr& v) {
...
const NodePtr& TypelessVarDefExpr::getTypeExpr(bool required) {
...
void TypelessVarDefExpr::setInitialExpr(const NodePtr& v) {
...
const NodePtr& TypelessVarDefExpr::getInitialExpr(bool required) {
...
void TypelessVarDefExpr::encode(Encoder& e) {
...
void TypelessVarDefExpr::decode(Decoder& d) {
...
void TypelessVarDefExpr::updateHierarchyOfChildren() {
...
const NodePtr& TypelessVarDefExpr::resolvePath(const std::string& path) {
...
NodeVector TypelessVarDefExpr::getChildren() {
	NodeVector v;
