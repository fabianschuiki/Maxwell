...
public:
	...
	virtual std::string getClassName() const { return "IndexOpExpr"; }
	...
	template<typename T> static Ptr needFrom(const T& n) {
		Ptr r = boost::dynamic_pointer_cast<IndexOpExpr>(n);
		if (!r)
			throw std::runtime_error("Node " + n->getId().str() + " (a " + n->getClassName() + ") cannot be dynamically cast to IndexOpExpr.");
		return r;
	}
