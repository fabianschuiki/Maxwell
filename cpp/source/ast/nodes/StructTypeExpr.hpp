...
class StructTypeExpr : public Node
{
public:
	StructTypeExpr() : Node(),
		interfaceGraph(this),
	...
	virtual string getClassName() const { return "StructTypeExpr"; }
	...
	{
		if (depth == 0) return "StructTypeExpr{…}";
		stringstream str, b;
		str << "StructTypeExpr{";
	...
	{
		const shared_ptr<StructTypeExpr>& other = boost::dynamic_pointer_cast<StructTypeExpr>(o);
	...
	typedef boost::shared_ptr<StructTypeExpr> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<StructTypeExpr>(n); }
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<StructTypeExpr>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to StructTypeExpr."); return r; }
protected:
	NodeRef graphPrev;
	...
	// Interfaces
	GraphInterfaceImpl<StructTypeExpr> interfaceGraph;
	TypeExprInterfaceImpl<StructTypeExpr> interfaceTypeExpr;
};
