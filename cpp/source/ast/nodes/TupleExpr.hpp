...
public:
	...
	virtual string getClassName() const { return "TupleExpr"; }
	...
	typedef boost::shared_ptr<TupleExpr> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<TupleExpr>(n); }
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<TupleExpr>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to TupleExpr."); return r; }
protected:
	NodeRef graphPrev;
	...
	// Interfaces
	GraphInterfaceImpl<TupleExpr> interfaceGraph;
	TypeInterfaceImpl<TupleExpr> interfaceType;
};
