...
class SetConstExpr : public Node
{
public:
	SetConstExpr() : Node(),
		interfaceType(this),
	...
	virtual string getClassName() const { return "SetConstExpr"; }
	...
	typedef boost::shared_ptr<SetConstExpr> Ptr;
	template<typename T> static Ptr from(const T& n) { return boost::dynamic_pointer_cast<SetConstExpr>(n); }
	template<typename T> static Ptr needFrom(const T& n) { Ptr r = boost::dynamic_pointer_cast<SetConstExpr>(n); if (!r) throw std::runtime_error("Node " + n->getId().str() + " cannot be dynamically casted to SetConstExpr."); return r; }
protected:
	NodeRef graphPrev;
	...
	// Interfaces
	TypeInterfaceImpl<SetConstExpr> interfaceType;
