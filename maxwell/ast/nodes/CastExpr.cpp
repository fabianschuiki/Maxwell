...
bool CastExpr::isKindOf(Kind k) {
	if (Node::isKindOf(k)) return true;
	return k == kCastExpr;
}

bool CastExpr::implements(Interface i) {
...
bool CastExpr::equalTo(const NodePtr& o) {
	const shared_ptr<CastExpr>& other = boost::dynamic_pointer_cast<CastExpr>(o);
...
std::string CastExpr::describe(int depth) {
	std::stringstream str, b;
	if (depth == 0) return "CastExpr{…}";
	str << "CastExpr{";
...
void CastExpr::setGraphPrev(const NodePtr& v) {
	if (!v && graphPrev) {
		modify("graphPrev");
		graphPrev.reset();
	}
...
void CastExpr::setGraphPrev(const NodeId& v) {
	if (v != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
...
const NodePtr& CastExpr::getGraphPrev(bool required) {
...
void CastExpr::setPossibleType(const NodePtr& v) {
...
const NodePtr& CastExpr::getPossibleType(bool required) {
...
void CastExpr::setRequiredType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType) && !v->isKindOf(kNativeType) && !v->isKindOf(kMutableType) && !v->isKindOf(kMutableCastType) && !v->isKindOf(kImmutableCastType)) {
...
const NodePtr& CastExpr::getRequiredType(bool required) {
...
void CastExpr::setActualType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType) && !v->isKindOf(kNativeType) && !v->isKindOf(kMutableType) && !v->isKindOf(kMutableCastType) && !v->isKindOf(kImmutableCastType)) {
...
const NodePtr& CastExpr::getActualType(bool required) {
...
void CastExpr::setTypeExpr(const NodePtr& v) {
...
const NodePtr& CastExpr::getTypeExpr(bool required) {
...
void CastExpr::setExpr(const NodePtr& v) {
	if (!equal(v, expr)) {
		modify("expr");
		expr = v;
	}
...
const NodePtr& CastExpr::getExpr(bool required) {
...
void CastExpr::encode(Encoder& e) {
...
void CastExpr::decode(Decoder& d) {
...
void CastExpr::updateHierarchyOfChildren() {
...
const NodePtr& CastExpr::resolvePath(const std::string& path) {
...
NodeVector CastExpr::getChildren() {
	NodeVector v;
