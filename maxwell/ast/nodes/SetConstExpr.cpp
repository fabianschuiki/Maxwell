...
#include <sstream>
#include <stdexcept>
using ast::SetConstExpr;
using ast::NodePtr;
using ast::NodeVector;

SetConstExpr::SetConstExpr() : Node(),
	interfaceGraph(this),
	interfaceType(this) {}

bool SetConstExpr::isKindOf(Kind k) {
	if (Node::isKindOf(k)) return true;
	return k == kSetConstExpr;
}

bool SetConstExpr::implements(Interface i) {
...
NodePtr SetConstExpr::copy() {
	Ptr c (new SetConstExpr);
...
bool SetConstExpr::equalTo(const NodePtr& o) {
	const shared_ptr<SetConstExpr>& other = boost::dynamic_pointer_cast<SetConstExpr>(o);
...
std::string SetConstExpr::describe(int depth) {
	std::stringstream str, b;
	if (depth == 0) return "SetConstExpr{…}";
	str << "SetConstExpr{";
...
void SetConstExpr::setGraphPrev(const NodePtr& v) {
	if (!v && graphPrev) {
		modify("graphPrev");
		graphPrev.reset();
	}
...
void SetConstExpr::setGraphPrev(const NodeId& v) {
	if (v != graphPrev.id) {
		modify("graphPrev");
		graphPrev.set(v);
	}
...
const NodePtr& SetConstExpr::getGraphPrev(bool required) {
...
void SetConstExpr::setPossibleType(const NodePtr& v) {
...
const NodePtr& SetConstExpr::getPossibleType(bool required) {
...
void SetConstExpr::setRequiredType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType) && !v->isKindOf(kNativeType) && !v->isKindOf(kMutableType) && !v->isKindOf(kMutableCastType) && !v->isKindOf(kImmutableCastType)) {
...
const NodePtr& SetConstExpr::getRequiredType(bool required) {
...
void SetConstExpr::setActualType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType) && !v->isKindOf(kNativeType) && !v->isKindOf(kMutableType) && !v->isKindOf(kMutableCastType) && !v->isKindOf(kImmutableCastType)) {
...
const NodePtr& SetConstExpr::getActualType(bool required) {
...
void SetConstExpr::setExprs(const NodeVector& v) {
	if (!equal(v, exprs)) {
		modify("exprs");
		exprs = v;
	}
...
const NodeVector& SetConstExpr::getExprs(bool required) {
	const NodeVector& v = exprs;
	return v;
}


void SetConstExpr::setExprsType(const NodePtr& v) {
	if (v && !v->isKindOf(kGenericType) && !v->isKindOf(kInvalidType) && !v->isKindOf(kNilType) && !v->isKindOf(kDefinedType) && !v->isKindOf(kUnionType) && !v->isKindOf(kTupleType) && !v->isKindOf(kFuncType) && !v->isKindOf(kTypeSet) && !v->isKindOf(kQualifiedType) && !v->isKindOf(kSpecializedType) && !v->isKindOf(kUnionMappedType) && !v->isKindOf(kOneTupleMappedType) && !v->isKindOf(kCastType) && !v->isKindOf(kNativeType) && !v->isKindOf(kMutableType) && !v->isKindOf(kMutableCastType) && !v->isKindOf(kImmutableCastType)) {
...
const NodePtr& SetConstExpr::getExprsType(bool required) {
...
void SetConstExpr::encode(Encoder& e) {
...
void SetConstExpr::decode(Decoder& d) {
...
void SetConstExpr::updateHierarchyOfChildren() {
...
const NodePtr& SetConstExpr::resolvePath(const std::string& path) {
...
NodeVector SetConstExpr::getChildren() {
	NodeVector v;
